...
Hash table of symbol id to ‘agent-shell-prompt-spec’. Populate via ~agent-shell-prompt-def~.
...
*** [[file:../agent-shell-prompt.el::92][agent-shell-prompt-def]]
:PROPERTIES:
:CUSTOM_ID: agent-shell-prompt-def
:END:
