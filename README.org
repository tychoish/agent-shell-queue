...
Add a new active item for PROMPT destined for BUF.  When BACKGROUND is
non-nil, the item is flagged for sub-agent execution.  Saves and refreshes.
...
Replace the prompt text of item ID with NEW-PROMPT.
...
Dispatch item ID to its assigned session immediately.
