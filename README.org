...
- [[#user-guides][User Guides]]
  ...
  - [[#execution-control-and-flow-modifiers][Execution Control and Flow Modifiers]]
...
- [[#api-reference][API Reference]]
  ...
  - [[#agent-shell-queue][agent-shell-queue]]
    - [[#prompt-composition-and-enqueueing][Prompt Composition and Enqueueing]]
    - [[#execution-control-and-flow-modifiers][Execution Control and Flow Modifiers]]
    - [[#display-and-column-options][Display and Column Options]]
    - [[#queue-buffer-and-navigation][Queue Buffer and Navigation]]
    - [[#queue-flow-modifiers-and-wait-items][Queue Flow Modifiers and Wait Items]]
    - [[#item-view-and-raw-inspection][Item View and Raw Inspection]]
    - [[#prompt-editing-and-direct-state-modification][Prompt Editing and Direct State Modification]]
    - [[#session-forking-and-worktrees][Session Forking and Worktrees]]
    - [[#task-interjection][Task Interjection]]
...
- *Targeted and Unassigned Capture*: Compose prompts destined for a specific agent shell or park them in the =(unassigned)= queue to assign later.
- *Context and Region Seeding*: Capture prompts seeded from the active region, kill-ring, clipboard, or thing-at-point.
- *Org-Mode Heading Refile*: Ingest task trees directly from Org files into the prompt queue.
- *Directory-Scoped Queues*: Queue items for directories without an active shell; shells spawn automatically upon dispatch.
- *Alternative Execution Kinds*: Queue Emacs Lisp forms, interactive commands, eshell, or eat terminal commands.
...
(agent-shell-queue-enqueue "Refactor auth middleware and add test cases"
                           (get-buffer "*agent-shell: backend*")
                           nil  ; background flag
                           5.0  ; delay 5s before sending
                           2.0) ; delay 2s after turn completes

;; Directory queue: creates shell on dispatch automatically
(agent-shell-queue-add-directory "Run project linting and format checks"
                                 "~/src/project-b/")
#+END_SRC
...
(agent-shell-queue-insert-wait (current-buffer))
#+END_SRC
...
;; Keybindings in *agent-shell-queue* buffer:
;; RET   - View item details
;; e/C-e - Enqueue dispatch / Edit task prompt
;; p/r   - Pause / Resume item
;; M-up  - Move item earlier in queue
;; M-down- Move item later in queue
;; C-d x - Raw YAML queue editor
#+END_SRC
...
(agent-shell-queue-fork-session (current-buffer)
                                "q0abcd"
                                '(:fork-mode fork
                                  :use-worktree t
                                  :worktree-branch "feature/experimental-branch"))
#+END_SRC
...
(setq agent-shell-queue-safe-save t
      agent-shell-queue-safe-save-format 'yaml
