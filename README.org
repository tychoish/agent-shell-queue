...
#+BEGIN_COMMENT
Generated from docstrings in agent-shell-queue.el. To regenerate: open this
file in Emacs with agent-shell-queue on your load-path, place point in the
...
Change el-file to agent-shell-menu.el to document the menu module instead.
...
#+BEGIN_SRC emacs-lisp :results output raw :exports none
  ...
  (let* ((org-dir (file-name-directory (buffer-file-name)))
         ;; Change to agent-shell-menu.el to document the menu module.
         (el-file (expand-file-name "agent-shell-queue.el" org-dir))
    ...
    (defun asq-doc--format-sym (name &optional heading)
      (let* ((sym (intern name))
             (fn-p (fboundp sym))
    ...
    (cond
     ...
     ;; rather than long ruler lines; the section name is on the same line.
